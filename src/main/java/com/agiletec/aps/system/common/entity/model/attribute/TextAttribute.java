...
public class TextAttribute extends AbstractTextAttribute {
    ...
    public String getText() {
        ...
        if (StringUtils.isBlank(text)) {
            ...
            if (StringUtils.isBlank(text)) {
                // MODIFICATO PER GIUSTIZIA / ALMAVIVA
                text = (String) this.getTextMap().get("en");
                if (StringUtils.isBlank(text)) {
                    text = "";
                }
            }
    ...
    public String getTextForLang(String langCode) {
        String text = this.getTextMap().get(langCode);
        // MODIFICATO PER GIUSTIZIA / ALMAVIVA
        if (StringUtils.isBlank(text) && "it".equals(langCode)) {
            text = this.getTextMap().get("en");
        }
        return text;
    }
    ...
    public List<AttributeSearchInfo> getSearchInfos(List<Lang> systemLangs) {
        ...
        if (null != this.getTextMap() && this.getTextMap().size() > 0) {
            ...
            for (int i = 0; i < systemLangs.size(); i++) {
                ...
                // MODIFICATO PER GIUSTIZIA / ALMAVIVA
                if (StringUtils.isBlank(text) && "it".equals(lang.getCode())) {
                    text = this.getTextMap().get("en");
                }
                // MODIFICATO PER GIUSTIZIA / ALMAVIVA - fine
                if (null != text) {
