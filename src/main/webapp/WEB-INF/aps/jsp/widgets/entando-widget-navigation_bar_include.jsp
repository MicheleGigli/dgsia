...
<c:if test="${previousLevel < level}">
	...
	<c:if test="${previousLevel > 0}">
		<c:set var="liClass" value=' class="nav-item dropdown-submenu"' />
		<c:if test="${previousPage.code == currentPageCode}">
			<c:set var="liClass" value=' class=" nav-item dropdown-submenu active"' />
		</c:if>
	...
	<li<c:out value="${liClass}" escapeXml="false" />><a href="<c:out value="${aURL}" escapeXml="false" />"<c:out value="${aClassAndData}" escapeXml="false" />><!-- [ <c:out value="${previousLevel}" /> ] --><c:out value="${homeIcon}" escapeXml="false" /><span><c:out value="${previousPage.title}" /></span><c:out value="${caret}" escapeXml="false" /></a>
